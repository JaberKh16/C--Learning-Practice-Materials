...
class WorkWithFunction
{
    ...
    public int CallingNullableParams(string? name, int? count = 0)
    {
        Console.WriteLine("Enter the count value: ");
        var count = System.Int32(Console.ReadLine());
        if(count == 1){
            if(string.IsNullOrEmpty(name)){
                name = "Default";
                // name ??="Default"; // with null coelsing opertor assinging default value
            }
            Console.WriteLine($"Provided Name is : ${name}");
        }
    }
    ...
    public static void Main(string[] args)
    {
        ...
        switch (choice)
        {
            ...
            case "4":
                ...
                break;
            case "5":
                CallingNullableParams(string? name, int? count = 0);
                break;
            default:
